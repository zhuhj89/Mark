...
agent = Module(__name__)

@mark_app.route('/agent/')
@login_required
def agent_index():
    """to index page.
...
@mark_app.route('/agent/add', methods=['GET', 'POST'])
@login_required
def add_agent():
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            domain_price_id = form.data['domain_price_id']
            url = form.data['url']
            ...
            db.session.commit()
            flash(u'添加成功')
            return redirect(url_for('query_agent_list', agent_name=agent_name))
    return render_template('/agent/add_agent.html', form=form)

@mark_app.route('/agent/update/<id>/', methods=['GET', 'POST'])
@login_required
def update_agent(id):
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            db.session.commit()
            flash(u'修改成功')
            return redirect(url_for('query_agent_list', agent_name=agent.agent_name))
    return render_template('/agent/update_agent.html', form=form, agent=agent)

@mark_app.route('/agent/using/<id>/<status>/', methods=['GET', 'POST'])
@login_required
...
@mark_app.route('/agent/list', methods=['GET', 'POST'])
@mark_app.route('/agent/list/<int:page>/')
@mark_app.route('/agent/list/<agent_name>/<int:page>/')
@login_required
def query_agent_list(agent_name='', page=1):
    ...
    if agent_name:
        page_url = lambda page: url_for('query_agent_list', agent_name=agent_name, page=page)
    else:
        page_url = lambda page: url_for('query_agent_list', page=page)
    return render_template('/agent/agent_list.html', agent_name=agent_name, page_obj=page_obj, page_url=page_url)

@mark_app.route('/agent/account', methods=['GET', 'POST'])
@login_required
def query_account():
    """to agent account page.
...
@mark_app.route('/agent/pay-detail/<int:page>/', methods=['GET', 'POST'])
@mark_app.route('/agent/pay-detail/<agent_name>/<sdate>/<edate>/<int:page>/', methods=['GET', 'POST'])
@mark_app.route('/agent/pay-detail', methods=['GET', 'POST'])
@login_required
    ...
    if agent_name and sdate and edate:
        ...
        if not q_agent:
            ...
            page_obj = DomainPayRecord.query.filter(DomainPayRecord.agent_id == q_agent.id,
                                                    DomainPayRecord.create_date >= sdate,
                                                    DomainPayRecord.create_date <= edate)\
                .paginate(page, per_page=page_count)

            if not page_obj.items:
                flash(u'没有消费记录')
            page_url = lambda page: url_for('query_pay_detail',agent_name=agent_name, sdate=sdate,
...
@mark_app.route('/agent/update-remind/<id>/', methods=('GET', 'POST'))
@login_required
def update_remind(id):
    ...
    if request.method == 'POST':
        ...
        if form.validate_on_submit():
            ...
            return redirect(url_for('query_agent_list', agent_name=agent.agent_name))
    return render_template('/agent/update_remind.html', form=form)

@mark_app.route('/agent/update-price/<id>/', methods=['GET', 'POST'])
@login_required
def update_price(id):
...
@mark_app.route('/agent/charge/<id>/', methods=['GET', 'POST'])
@login_required
def charge(id):
    """to charge page.
...
@mark_app.route('/agent/payment/<id>/', methods=['GET', 'POST'])
@login_required
def payment(id):
    """to payment page.
